...
# turn's (avatar_image, audio_wav) pair, producing per-turn MP4s.
#
...
param(
    ...
    [Parameter(Mandatory=$false)]
    [switch]$Still        # if set, restricts motion for more stable output
)
...
if (-not (Test-Path $venvPath)) {
    Write-Host "✗ .venv-sadtalker not found. Run .\scripts\install_sadtalker.ps1 first." -ForegroundColor Red
    exit 1
}
if (-not (Test-Path $sadTalkerDir)) {
    Write-Host "✗ SadTalker/ directory not found. Run .\scripts\install_sadtalker.ps1 first." -ForegroundColor Red
    exit 1
}
if (-not (Test-Path $inferScript)) {
    Write-Host "✗ inference.py not found at $inferScript. SadTalker structure may have changed." -ForegroundColor Red
    exit 1
}
...
if (-not $AudioDir) {
    $audioDir = Join-Path (Split-Path $debatePath -Parent) ((Split-Path $debatePath -LeafBase) + "_audio")
} else {
    $audioDir = (Resolve-Path $AudioDir).Path
}
if (-not $OutDir) {
    $outDir = Join-Path (Split-Path $debatePath -Parent) ((Split-Path $debatePath -LeafBase) + "_anim")
}

if (-not (Test-Path $audioDir)) {
    Write-Host "✗ Audio directory not found: $audioDir" -ForegroundColor Red
...
foreach ($turn in $turns) {
    ...
    $t0 = Get-Date
    # SadTalker runs from its own directory (relative path imports)
    ...
    try {
        ...
        if ($proc.ExitCode -eq 0) {
            ...
            Write-Host "✓ done in $($elapsed.TotalSeconds)s" -ForegroundColor Green
            $success++
        } else {
            Write-Host "✗ failed (exit $($proc.ExitCode))" -ForegroundColor Red
            Write-Host "    see log: $outDir\render_$idx.log" -ForegroundColor Gray
            Write-Host "    see err: $outDir\render_$idx.err" -ForegroundColor Gray
